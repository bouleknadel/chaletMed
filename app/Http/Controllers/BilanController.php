...
class BilanController extends Controller
{
    public function calculateTotals(Request $request)
    {
        ...
        $balance_credit = Balance::where('debit', 0)->where('annee', $anneeSelectionnee)->sum('montant');
        $cotisationsPayeesAnneeEnCours += $balance_credit;
        $balance_debit = Balance::where('debit', 1)->where('annee', $anneeSelectionnee)->sum('montant');
        $cotisationsPayeesAnneeEnCours -= $balance_debit;
